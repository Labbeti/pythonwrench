...
class TestIsInstanceGuard(TestCase):
    ...
    def test_old_compatibility(self) -> None:
        ...
        for x in examples:
            assert old_is_dict_str(x) == isinstance_generic(x, Dict[str, Any])
            assert old_is_dict_str_number(x) == isinstance_generic(x, Dict[str, Number])
            assert old_is_dict_str_optional_int(x) == isinstance_generic(
                x, Dict[str, Optional[int]]
            )
            assert old_is_dict_str_str(x) == isinstance_generic(x, Dict[str, str])

            assert old_is_iterable_bool(x) == isinstance_generic(x, Iterable[bool])
            assert old_is_iterable_bytes_or_list(x) == isinstance_generic(
                x, Iterable[Union[bytes, list]]
            )
            assert old_is_iterable_float(x) == isinstance_generic(x, Iterable[float])
            assert old_is_iterable_int(x) == isinstance_generic(x, Iterable[int])
            assert old_is_iterable_integral(x) == isinstance_generic(
                x, Iterable[Integral]
            )
            assert old_is_iterable_iterable_int(x) == isinstance_generic(
                x, Iterable[Iterable[int]]
            )
            assert old_is_iterable_mapping_str(x) == isinstance_generic(
                x, Iterable[Mapping[str, Any]]
            )
            assert old_is_iterable_str(x) == isinstance_generic(x, Iterable[str])

            assert old_is_list_bool(x) == isinstance_generic(x, List[bool])
            assert old_is_list_builtin_number(x) == isinstance_generic(
                x, List[BuiltinNumber]
            )
            assert old_is_list_float(x) == isinstance_generic(x, List[float])
            assert old_is_list_int(x) == isinstance_generic(x, List[int])
            assert old_is_list_list_str(x) == isinstance_generic(x, List[List[str]])
            assert old_is_list_number(x) == isinstance_generic(x, List[Number])
            assert old_is_list_str(x) == isinstance_generic(x, List[str])

            assert old_is_mapping_str(x) == isinstance_generic(x, Mapping[str, Any])
            assert old_is_mapping_str_iterable(x) == isinstance_generic(
                x, Mapping[str, Iterable]
            )
            assert old_is_namedtuple_instance(x) == isinstance_generic(
                x, NamedTupleInstance
            )

            assert old_is_sequence_bool(x) == isinstance_generic(x, Sequence[bool])
            assert old_is_sequence_int(x) == isinstance_generic(x, Sequence[int])
            assert old_is_sequence_str(x) == isinstance_generic(x, Sequence[str])

            assert old_is_tuple_optional_int(x) == isinstance_generic(
                x, Tuple[Optional[int], ...]
            )
            assert old_is_tuple_int(x) == isinstance_generic(x, Tuple[int, ...])
            assert old_is_tuple_str(x) == isinstance_generic(x, Tuple[str, ...])
